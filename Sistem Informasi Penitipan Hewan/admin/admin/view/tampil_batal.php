...
$tampil = mysqli_query($koneksi,'SELECT*FROM transaksi INNER JOIN user ON transaksi.`id_user`=user.`id_user` WHERE transaksi.`status`=2;');
...
<div style="margin-top: 15px;margin-left: 15px;margin-right: 15px">
	<h4>Data Transaksi Dibatalkan</h4>
...
<script type="text/javascript">
	...
	$(document).ready( function () {
		...
		// 						Swal.fire({
		// 							title: 'Hapus Sukses',
		// 							text: 'Transaksi Penitipan Berhasil DiHapus',
		// 							type: 'success',
		// 						}).then((result) => {
		// 							if (result.value) {
		// 								$('#'+id_tr).remove();
