...
<div id="konten">
			...
			<form method="post" action="#">
				<div class="col-md-5">
					...
					<center>
						...
						<hr>
						<button id="ulang" type="submit" name="ulang" class="btn btn-info"><i class=""></i>Ulangi</button>
						<button id="tunda" type="submit" value="Tunda" name="tunda" class="btn btn-danger">Tunda</button>
	...
	<?php 
	$cek_akhir_nomor = mysqli_query($koneksi,"SELECT COUNT(no_antrian) as x FROM pasien_berobat WHERE tanggal_berobat=CURDATE() AND tujuan='bpumum' AND  status='Belum Selesai'");
	$result1 = $cek_akhir_nomor->fetch_assoc();

	$cek_awal_nomor = mysqli_query($koneksi,"SELECT COUNT(no_antrian) as y FROM pasien_berobat WHERE tanggal_berobat=CURDATE() AND tujuan='bpumum' AND  status='Selesai'");
	$result2 = $cek_awal_nomor->fetch_assoc();

	if ($result1['x']==0) {?>

		<script type="text/javascript">swal("Antrian BP Umum Selesai", "Tidak ada antrian selanjutnya", "success");</script>
