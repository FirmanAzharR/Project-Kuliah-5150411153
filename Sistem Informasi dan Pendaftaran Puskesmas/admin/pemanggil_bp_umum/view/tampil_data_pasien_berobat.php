...
<table class="myTable table table-hover table-bordered">
	...
	<tbody>
		<?php $data = mysqli_query($koneksi,"SELECT*FROM pasien_berobat WHERE tanggal_berobat=CURDATE() AND tujuan='bpumum'") ?>
...
<label>BP Umum ( Status : Selesai ) : 
	<?php 
	$x = mysqli_query($koneksi,"SELECT no_rm FROM pasien_berobat WHERE tanggal_berobat=CURDATE() AND tujuan='bpumum' AND status='Selesai'");
	echo mysqli_num_rows($x); 
	?> Pasien
</label><br>
<label>BP Umum ( Status : Belum Selesai ) : 
	<?php 
	$x = mysqli_query($koneksi,"SELECT no_rm FROM pasien_berobat WHERE tanggal_berobat=CURDATE() AND tujuan='bpumum' AND status='Belum Selesai'");
	echo mysqli_num_rows($x); 
	?> Pasien
</label>
