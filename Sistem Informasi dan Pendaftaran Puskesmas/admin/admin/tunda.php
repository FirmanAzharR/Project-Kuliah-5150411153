...
<div id="konten">
	...
	<table class="myTable table table-hover table-bordered">
		...
		<tbody>
			<?php $data = mysqli_query($koneksi,"SELECT*FROM pasien_berobat WHERE status='Tunda' AND tanggal_berobat=CURDATE() AND tujuan='bpgigi'") ?>
			<?php foreach ($data as $key => $value): ?>
				<tr align="center">
					<td><?php echo $key+1; ?></td>
					<td><?php echo $value['nama_pasien']; ?></td>
					...
					<td>
						<a href="proses/ubah_status_selesai2.php?norm=<?php echo $value['no_rm'] ?>" class="btn btn-sm btn-success" id="selesai" data-role="selesai" data-id="<?php echo $value['no_rm'] ?>">Selesai</a>
						<a href="proses/ubah_status_batal2.php?norm=<?php echo $value['no_rm'] ?>" class="btn btn-sm btn-danger" id="gagal" data-role="gagal" data-id="<?php echo $value['no_rm'] ?>">Batal</a>
					</td>
				</tr>
			<?php endforeach ?>
		</tbody>
